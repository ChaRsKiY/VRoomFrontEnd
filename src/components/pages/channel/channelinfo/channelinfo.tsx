...
const ChannelInfoComponent : React.FC<IProps> =({ channelid }) => {
    ...
    const checkIsFolowed = async ( ) => {
        if(user){ 
        try {     
          const response = await fetch('https://localhost:7154/api/Subscription/isfolowed/'+channelid +'/'+ user.id , {
            method: 'GET',
          });
      ...
      const addSubscription = async ( ) => {
        if(user){ 
        try {     
          const response = await fetch('https://localhost:7154/api/Subscription/add/'+channelid +'/'+ user.id , {
            method: 'POST',
          });
      ...
      const deleteSubscription = async ( ) => {
        if(user){ 
        try {     
          const response = await fetch('https://localhost:7154/api/Subscription/delete/'+channelid +'/'+ user.id , {
            method: 'DELETE',
          });
    ...
    return (
        ...
        <div style={{marginBottom:'100px',width:'100%'}}  >
            ...
            <button onClick={showMain} style={{padding:'5px',fontWeight:font1,
                marginLeft:'20px',margin:'2px',fontSize:"16px",borderBottom:border1,marginBottom:'0'}}>Main 
                </button>
            <button onClick={showAll} style={{padding:'5px',fontWeight:font2,
                marginLeft:'20px', margin:'2px',fontSize:"16px",borderBottom:border2,marginBottom:'0'}}>Video</button>
                 <button onClick={showLives} style={{padding:'5px',fontWeight:font5,
                marginLeft:'20px', margin:'2px',fontSize:"16px",borderBottom:border5,marginBottom:'0'}}>Lives</button>
            <button onClick={showPosts} style={{padding:'5px',fontWeight:font3,
                marginLeft:'20px', margin:'2px',fontSize:"16px",borderBottom:border3,marginBottom:'0'}}>Communities</button>
                 <button onClick={showPlaylists} style={{padding:'5px',fontWeight:font6,
                marginLeft:'20px', margin:'2px',fontSize:"16px",borderBottom:border6,marginBottom:'0'}}>Playlists</button>
                 <button onClick={showAbout} style={{padding:'5px',fontWeight:font4,
                marginLeft:'20px', margin:'2px',fontSize:"16px",borderBottom:border4,marginBottom:'0'}}>About</button>
            <hr />
        ...
        onClick={showAllViwed}>For you &nbsp;&nbsp;
         <FaPlay size={12} className="text-black-600 cursor-pointer" title='All popular' style={{display:'inline'}}/></button>
            <div className="grid pr-[2%] grid-cols-4 grid-rows-1 flex-1 max-lg:grid-cols-2 max-sm:grid-cols-1 max-sm:pr-0">
         ...
         <FaPlay size={12} className="text-black-600 cursor-pointer" title='All popular' style={{display:'inline'}} /></button>
            ...
            {videosShorts.slice(0,4).map((el, key) => (
             ...
             <hr />           
              ...
              <div style={{padding:"10px",fontSize:'20px',fontWeight:'bold'}}>Honourable mentions</div>
              <div className='flex' >
             {mentors.map((el, key) => (
                <div key={key} className="px-3 mb-8 space-y-2.5">
                  <div style={{display:'flex',flexDirection:'column'}}>
                     <div style={{display:'flex',justifyContent:'center'}}><Image src={el.channelProfilePhoto} alt="Banner Image" width={60} height={60 } 
                     className='rounded-full' style={{minHeight:'60px',maxHeight:'60px'}}/> </div>
                     <div style={{fontSize:'12px'}}>{el.channelNikName}</div>
                     <div style={{display:'flex',justifyContent:'center'}}>
                      <Link href={el.channel_URL} style={{ padding:'2px',backgroundColor:'lightgray',
                        paddingLeft:'10px',paddingRight:'10px',borderRadius:'5px',marginTop:'5px' }}>Follow</Link>
                      </div>
                     </div>
                </div>
            ))}
        ...
        <div onClick={showAll} style={{display:display7, marginTop:'20px' }} >
          ...
          <button onClick={showLatest} style={{padding:'5px',borderRadius:'5px',
            ...
            <button onClick={showOldest} style={{padding:'5px',borderRadius:'5px',
                marginLeft:'20px', margin:'2px',fontSize:"16px",
                backgroundColor:color3, color:color4}}>Oldest</button>
                 <button onClick={showPop} style={{padding:'5px',borderRadius:'5px',
                marginLeft:'20px', margin:'2px',fontSize:"16px",
                backgroundColor:color5, color:color6}}>Popular</button>
