...
const Notifications = () => {
  ...
  const fetchNotifications = async (userId: string) => {
    try {
      console.log('Fetching notifications***************************'); // Лог для проверки, что функция вызывается
      // Добавляем параметр времени, чтобы предотвратить кэширование
      const response = await fetch(`https://localhost:7154/api/Notification/getbyuserid/1/10/${userId}?timestamp=${new Date().getTime()}`);
      ...
      console.log('Fetched notifications:', notifications); // Лог уведомлений
  ...
  useEffect(() => {
    if (user) {
      // Загружаем уведомления при первом рендере
      console.log('Fetching not????***************************');
      ...
      console.log('Interval set:!!!!!!!!!!!!', intervalId); // Лог установки интервала

      // Очищаем интервал при размонтировании компонента
      return () => {
        console.log('Clearing interval:!!!!!!!!!!!', intervalId); // Лог очистки интервала
        clearInterval(intervalId);
      };
    }
  }, [user]); // Зависимость - пользователь
