...
const UnlimitedScrollBlock = ({ data }: { data: IPresentedVideo[] }) => {
    return (
        <div className="grid pr-[2%] grid-cols-3 flex-1 max-lg:grid-cols-2 max-sm:grid-cols-1 max-sm:pr-0 lgxl:grid-cols-4 lgxxl:grid-cols-5">
            {data.map((el) => (
                <Link href={"/watch/" + el.href} key={el.id} className="px-3 mb-8 space-y-2.5">
