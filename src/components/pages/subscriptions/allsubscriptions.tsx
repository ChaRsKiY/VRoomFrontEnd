...
const AllSubscriptionsComponent: React.FC = () => {
  ...
  const getSubscribers = async (channelId: number): Promise<number> => {
    ...
    try {
      const response = await api.get(`/Subscription/countbychannelid/${channelId}`);
      if (response.data) {
  ...
  const deleteSubscription = async (channelSettingsId: number, index: number) => {
    if (user) {
      try {
        const response = await api.delete('/Subscription/delete/' + channelSettingsId + '/' + user.id);
  ...
  return (
    <div style={{ width: '100%', marginLeft: '100%' }}>
      ...
      {allFollowed.length == 0 ? (<div className='flex ' style={{
